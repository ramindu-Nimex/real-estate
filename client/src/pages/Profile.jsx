...
export default function Profile() {
  ...
  const handleShowListing = async() => {
    try {
      ...
      const data = await res.json();
      if(data.success === false) {
        dispatch(setShowListingError(true));
        return;
      }
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} action="" className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>{fileUploadError ? (<span className='text-red-700'>Error Image Upload (image must be less than 2mb)</span>) :
        ...
        <input type="email" placeholder='E-MAIL' className='border p-3 rounded-lg' defaultValue={currentUser.email} id='email' onChange={handleChange}/>
        <input type="password" placeholder='PASSWORD' className='border p-3 rounded-lg' id='password' onChange={handleChange}/>
        <button disabled={loading} className='bg-slate-700 text-white rounded-lg p-3 uppercase hover:opacity-95 disabled:opacity-80'>{loading ? 'LOADING...' : 'UPDATE'}</button>
        <Link className='bg-green-700 text-white p-3 rounded-lg uppercase text-center hover:opacity-95 disabled:opacity-80' to={"/create-listing"}>
          CREATE LISTING
        </Link>
      </form>
      ...
      <p>{showListingError ? 'Error Showing Listings' : ''}</p>
        ...
        {userListings.map((listing) => <div key={listing._id} className='border rounded-lg p-3 flex justify-between items-center gap-4'>
          ...
          <Link className='text-slate-700 font-semibold flex-1 hover:underline truncate' to={`/listing/${listing._id}`}><p >{listing.name}</p></Link>
